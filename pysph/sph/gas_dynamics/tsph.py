...
class BalsaraSwitch(Equation):
    ...
    def post_loop(self, d_h, d_idx, d_cs, d_divv, d_gradv, d_alpha):
        ...
        curlv[0] = (d_gradv[9 * d_idx + 3 * 2 + 1] - d_gradv[
            9 * d_idx + 3 * 1 + 2])
        curlv[1] = (d_gradv[9 * d_idx + 3 * 0 + 2] - d_gradv[
            9 * d_idx + 3 * 2 + 0])
        curlv[2] = (d_gradv[9 * d_idx + 3 * 1 + 0] - d_gradv[
            9 * d_idx + 3 * 0 + 1])

        abscurlv = sqrt(
            curlv[0] * curlv[0] + curlv[1] * curlv[1] + curlv[2] * curlv[2])
...
class MomentumAndEnergy(Equation):
    ...
    def loop(self, d_idx, s_idx, d_m, s_m, d_p, s_p, d_cs, s_cs, d_rho, s_rho,
        ...
        if vijdotxij <= 0.0:
            ...
            d_ae[d_idx] -= 0.5 * (
                    VIJ[0] * avi[0] + VIJ[1] * avi[1] + VIJ[2] * avi[2])
